...
class MemMan:
    ...
    def process_request(self, cg: Change, p_fM: FileMan):
        assert self.pT.heap_size == len(self.p_m.avl_mem_slts) - self.p_m.avl_mem_slts.count()
    ...
    def evict_lru_page(self) -> Optional[int]:
        ...
        if self.pT.heap_size == u32Const.NUM_MEM_SLOTS.value:
    ...
    def evict_this_page(self, b_num: bNum_t):
        if self.blk_in_pg_tab(b_num):
            ...
            assert pg_tab_slot != self.pT.heap_size
    ...
    def rd_in_slot(self, b_num: bNum_t):
        ...
        assert p_t_slot != self.pT.heap_size
