...
class Journal:
    ...
    def _write_with_wraparound(self, data: bytes, dat_len: int, do_ct: bool, buf_sz: int, end_pt: int) -> int:
        """Write data that needs to wrap around to the beginning of the journal.

        Args:
            data: The bytes to write
            dat_len: Length of data to write
            do_ct: Whether to count bytes in total
            buf_sz: Size of the journal buffer
            end_pt: Endpoint of the write operation
    ...
    def _write_64bit_wraparound(self, data: bytes, under: int, over: int, do_ct: bool) -> int:
    ...
    def _write_32bit_wraparound(self, data: bytes, under: int, over: int, do_ct: bool) -> int:
    ...
    def _write_generic_wraparound(self, data: bytes, under: int, over: int, do_ct: bool) -> int:
    ...
    def _update_final_position(self, start_pos: int, bytes_written: int, data: bytes, do_ct: bool):
        """Update final position and log debug information if needed."""
        end_pos = self.js.tell()
        actual_bytes_written = end_pos - start_pos
        if do_ct:
            logger.debug(f"Wrote {actual_bytes_written} bytes for {data[:10]}...")

        self.final_p_pos = end_pos
