...
@pytest.mark.parametrize("crc_results,expected_writes,expected_blocks", [
    pytest.param(
        [True, False, True],  # Second block fails CRC
        1,  # Expected number of successful writes
        {0},  # Expected successfully processed blocks
        id="mid_block_crc_failure"
    ),
    pytest.param(
        [False, True, True],  # First block fails CRC
        0,  # No successful writes
        set(),  # No blocks should be processed
        id="first_block_crc_failure"
    ),
    pytest.param(
        [True, True, False],  # Last block fails CRC
        1,  # One successful write (of the full buffer)
        {0, 1},  # First two blocks should be processed
        id="last_block_crc_failure"
    )
])
def test_process_changes_crc_errors(journal, mocker, crc_results, expected_writes, expected_blocks):
    """Test handling of CRC errors during change processing."""
    # Mock disk operations
    mock_disk = mocker.patch.object(journal.sim_disk, 'get_ds')
    mock_disk().read.return_value = b'\0' * u32Const.BLOCK_BYTES.value

    # Create a change log with multiple blocks
    mock_cg_log = mocker.Mock(spec=ChangeLog)
    mock_cg_log.the_log = {
        0: [Change(0)],
        1: [Change(1)],
    ...
    # Setup
    mock_verify_crc = mocker.patch.object(journal, 'verify_page_crc')
    mock_verify_crc.side_effect = crc_results
    mock_write = mocker.patch.object(journal._change_log_handler, 'write_buffer_to_disk')
    ...
    # Verify which blocks were actually processed
    processed_blocks = set()
    for call_args in mock_disk().seek.call_args_list:
        block_num = call_args[0][0] // u32Const.BLOCK_BYTES.value
        processed_blocks.add(block_num)
    ...
    buffer_items = [item for item in journal._change_log_handler.pg_buf if item is not None]
    assert len(buffer_items) == 0


def test_process_changes_crc_calculation(journal, mocker):
    """Test that CRC is correctly calculated during change processing."""
    # Mock disk operations
    mock_disk = mocker.patch.object(journal.sim_disk, 'get_ds')
    mock_disk().read.return_value = b'\0' * u32Const.BLOCK_BYTES.value

    # Create a change that modifies multiple lines
    change = Change(0)
    test_data = b'A' * u32Const.BYTES_PER_LINE.value
    for i in range(3):
        change.add_line(i, test_data)

    mock_cg_log = mocker.Mock(spec=ChangeLog)
    mock_cg_log.the_log = {0: [change]}

    # Mock write_buffer_to_disk to capture the page before it's cleared
    original_write_buffer = journal._change_log_handler.write_buffer_to_disk
    captured_page = None

    def capture_page_and_write(*args, **kwargs):
        nonlocal captured_page
        for block_num, page in journal._change_log_handler.pg_buf:
            if block_num == 0:
                captured_page = page
                break
        return original_write_buffer(*args, **kwargs)

    journal._change_log_handler.write_buffer_to_disk = capture_page_and_write
    ...
    expected_crc = AJZlibCRC.get_code(
        captured_page.dat[:-u32Const.CRC_BYTES.value],
        u32Const.BYTES_PER_PAGE.value - u32Const.CRC_BYTES.value
    )

    # Extract actual CRC from page
