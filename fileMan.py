...
class FileMan:
    ...
    def delete_file(self, cli_id: int, i_num: inNum_t) -> bool:
        if self.iTbl.node_locked(i_num):
        ...
        b_n_list = self.iTbl.list_all_blk_n(i_num)
        for item in b_n_list:
            self.frLst.put_blk(item)
            self.p_mM.evict_this_page(item)

        self.iTbl.release_all_blk_n(i_num)
        self.iTbl.release_in_n(i_num)
    ...
    def count_files(self) -> inNum_t:
        ...
        for i in range(total_inodes):
            if not self.iTbl.avail.test(i):  # If the bit is not set, the inode is in use
                files += 1
        return files

    def count_blocks(self, i_num: inNum_t) -> bNum_t:
        return sum(1 for item in self.iTbl.ref_tbl_node(i_num).b_nums if item != SENTINEL_BNUM)

    def file_exists(self, i_num: inNum_t) -> bool:
        if i_num == SENTINEL_INUM:
            return False
        return self.iTbl.node_in_use(i_num)
    ...
    def block_exists(self, i_num: inNum_t, b_num: bNum_t) -> bool:
        ...
        if not self.file_exists(i_num):
            return False
        tmp = self.iTbl.ref_tbl_node(i_num)
        return b_num in tmp.b_nums
    ...
    def add_block(self, cli_id: int, i_num: inNum_t) -> bNum_t:
        ...
        if self.iTbl.assign_blk_n(i_num, b_num):
            self.p_mM.p_j.do_wipe_routine(b_num, self)
            print(f"{self.tabs(2, True)}Block {b_num} added to inode {i_num} for client {cli_id} at time {get_cur_time()}")
            return b_num
        else:
            print(f"Unable to add block to inode {i_num} for client {cli_id} at time {get_cur_time()}: no space in inode")
            return SENTINEL_BNUM

    def remv_block(self, cli_id: int, i_num: inNum_t, tgt: bNum_t) -> bool:
        if self.iTbl.node_locked(i_num):
            print(f"Unable to remove block {tgt} from inode {i_num} for client {cli_id} at time {get_cur_time()}: File locked.")
            return False

        if not self.iTbl.node_in_use(i_num):
            print(f"Unable to remove block {tgt} from inode {i_num} for client {cli_id} at time {get_cur_time()}: Inode not in use.")
            return False

        if self.iTbl.release_blk_n(i_num, tgt):
            self.remv_block_clean(tgt)
            print(f"{self.tabs(2)}Block {tgt} removed from inode {i_num} for client {cli_id} at time {get_cur_time()}")
            return True
        else:
            print(f"Unable to remove block {tgt} from inode {i_num} for client {cli_id} at time {get_cur_time()}: Block not found in inode")
            return False
